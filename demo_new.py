...
from models import AE_A_Parsec,CVAE,AE_AB_parsec,AE_A_Keypoint,AE_AB_keypoint
from utils import get_name,get_params,get_path,get_point2,point2img_new
...
modelA_p = AE_A_Parsec() # 编辑模型，input : source_params,target_params,source_keypoint     output: target_keypoint
modelB_p = CVAE() # 重建模型，input : target_params,target_keypoint   output: target_full
model_p = AE_AB_parsec(modelA_p,modelB_p) # input : source_params,target_params,source_keypoint     output: target_full
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
checkpoint = torch.load('weights/logs_edit_parsec_AB/cond_ckpt_epoch_1000.pth', map_location='cpu')
model_p.load_state_dict(checkpoint['model'], strict=True)
model_p.to(device)
model_p.eval()


modelA_k = AE_A_Keypoint() # 编辑模型，input : source_keypoint,target_keypoint,source_params     output: target_params
modelB_k = CVAE() # 重建模型，input : target_keypoint,target_params   output: target_full
model_k = AE_AB_keypoint(modelA_k,modelB_k) # input : source_keypoint,source_params,target_keypoint     output: target_full
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def infer(input_image,idx,slider0,slider1,slider2,slider3):
    ...
    # print('target_parsec_features:',target_parsec_features)
    # # 衡量一下和target_params的绝对误差和相对误差

    # for i in range(11):
    #     print('target_params:',target_params[0][i].cpu().numpy())
    #     print('target_parsec_features:',target_parsec_features[i])
    #     print('abs error:',abs(target_params[0][i].cpu().numpy()-target_parsec_features[i]))
    #     print('relative error:',abs(target_params[0][i].cpu().numpy()-target_parsec_features[i])/target_params[0][i].cpu().numpy())
    error = ''

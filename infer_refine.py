...
class Tester:
    ...
    def test(self,args):
        ...
        refine_iteration = 3
        recons_parsec_features_error = Counter()
        num = 0
        recons_error = 0
        ...
        for step, data in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                target_params_pred,ans = model.refine_forward4(source_keypoint, target_keypoint, source_params,target_params,refine_iteration=refine_iteration)  
                ...
                for i, ax in enumerate(axi):
                    ...
                    if i==0:
                      recons_error += l
                      for j in range(11):
                        recons_parsec_features_error[parsec_features[j]] += abs(f_pred.parsec_features[j] - f_gt.parsec_features[j])
                        num += 1
        ...
        with open(os.path.join(args.log_dir,'recons_parsec_features_error.txt'),'w') as f:
            for k,v in recons_parsec_features_error.items():
                f.write(f'{k}:{v/num}\n')
        with open(os.path.join(args.log_dir,'recons_error.txt'),'w') as f:
            f.write(f'{recons_error/(num*10**5)}\n')

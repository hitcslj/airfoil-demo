...
def infer(input_image,idx,slider0,slider1,slider2,slider3, slider_l, slider_r, slider_f):
    ...
    if len(global_points)==2:
        ...
        # TODO: 拖拽控制点之后再进行一下Bspline局部拟合

        #由于推理生成的点云连续性太差，使用bezier曲线拟合,进行平滑
        target_datt_new = target_point_pred[0].cpu().numpy().astype("float32")
        t_curve = np.linspace(0, 1, len(target_datt_new))
        target_datt_new = bezier_curve(target_datt_new, t_curve)
        # 设置缓冲区,进行拟合，然后拟合平滑之后再拼接
        indices_buff = get_indices(center_index, slider_l+10, slider_r+10, total_points)
        mask[indices_buff,:] = 1

        target_datt = target_datt_new[0] * mask + target_datt[0] * (1-mask)

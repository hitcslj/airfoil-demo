...
img_paths = get_path('data/airfoil/picked_uiuc_img2')
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Column():
        ...
        with gr.Row():
          with gr.Accordion("Physical parameters", open=False):
              ...
              slider3 = gr.Slider(0, 10, step=0.1,label=nameDict[3],value=1,visible=False)
              slider4 = gr.Slider(0, 10, step=0.1,label=nameDict[4],value=1,visible=False)
              slider5 = gr.Slider(0, 10, step=0.1,label=nameDict[5],value=1,visible=False)
              slider6 = gr.Slider(0, 10, step=0.1,label=nameDict[6],value=1,visible=False)
              slider7 = gr.Slider(0, 10, step=0.1,label=nameDict[7],value=1,visible=False)
              slider8 = gr.Slider(0, 10, step=0.1,label=nameDict[8],value=1,visible=False)
              slider9 = gr.Slider(0, 10, step=0.1,label=nameDict[9],value=1,visible=False)
          parsec_img = gr.Image(value='asserts/example_parsec.png')
    ...
    with gr.Row():
      ...
      img_in = gr.Image(label="Input Image",width=600,height=600)
      img_out = gr.Image(label="Output Image",width=600,height=600)
    with gr.Row():
        with gr.Row():
    ...
    gr.Examples(
        examples=['data/airfoil/picked_uiuc_img2/2032c.png','data/airfoil/picked_uiuc_img2/a18.png'],
        inputs=[img_in]
     )
    # Instruction
    with gr.Row():
